...
#import <ALBBOpenAccountUI/ALBBOpenAccountSmsLoginViewDelegate.h>
#import <ALBBOpenAccountUI/ALBBOpenAccountEmailFindPwdViewDelegate.h>
...
#import <ALBBOpenAccountUI/ALBBOpenAccountWebViewDelegate.h>
#import <ALBBOpenAccountUI/ALBBOpenAccountSSODelegate.h>
#import <ALBBOpenAccountUI/ALBBOpenAccountPreFindPwdViewDelegate.h>
#import <ALBBOpenAccountUI/ALBBOpenAccountEmailRegisterViewDelegate.h>

typedef NS_ENUM(NSUInteger, CombinedAccountType) {
  CombinedAccountTypeOpenAccount,
  CombinedAccountTypeTaobao
};

typedef NS_ENUM(NSUInteger, LoginAccountType) {
  LoginAccountTypeCombined,
  LoginAccountTypeTaobao
};

typedef void (^ALBBOpenAccountQrLoginSuccessCallback)(void);
typedef void (^ALBBOpenAccountQrLoginFailedCallback)(NSError *error);
typedef void (^ALBBOpenAccountCombinedLoginSuccessCallback)(CombinedAccountType accountType,NSDictionary *tbSession);
 ...
 *  邮箱注册
 */
- (void)showEmailRegisterInNavigationController:(UINavigationController *)navigationController
                                        success:(ALBBOpenAccountSuccessCallback)success
                                        failure:(ALBBOpenAccountFailedCallback)failure;
 ...
 *  邮箱注册（模态视图）
 */
- (void)presentEmailRegisterViewController:(UIViewController *)presentingViewController
                                   success:(ALBBOpenAccountSuccessCallback)success
                                   failure:(ALBBOpenAccountFailedCallback)failure;
 ...
 *  @param failure 找密失败回调，只有用户取消找密才会执行
 */
- (void)showEmailFindPasswordInNavigationController:(UINavigationController *)navigationController
                                            success:(ALBBOpenAccountSuccessCallback)success
                                            failure:(ALBBOpenAccountFailedCallback)failure;
 ...
 *  @param failure 找密失败回调，只有用户取消找密才会执行
 */
- (void)presentEmailFindPasswordViewController:(UIViewController *)presentingViewController
                                       success:(ALBBOpenAccountSuccessCallback)success
                                       failure:(ALBBOpenAccountFailedCallback)failure;
...
/**
 *  统一登录
 *  @param success 登陆成功回调，只有accountType==CombinedAccountTypeTaobao时，才会返回tbSession
 *  @param failure 登录失败回调，只有用户取消登录才会执行
 */
- (void)showCombinedLoginViewWithLoginAccountType:(LoginAccountType)loginType
                                          Success:(ALBBOpenAccountCombinedLoginSuccessCallback)success
                                          failure:(ALBBOpenAccountFailedCallback)failure;
...
- (void)setEmailRegisterViewDelegate:(id <ALBBOpenAccountEmailRegisterViewDelegate>)delegate;
- (void)setEmailFindPwdViewDelegate:(id <ALBBOpenAccountEmailFindPwdViewDelegate>)delegate;
...
- (void)setPreFindPwdViewDelegate:(id<ALBBOpenAccountPreFindPwdViewDelegate>)delegate;
- (void)setWebViewDelegate:(id <ALBBOpenAccountWebViewDelegate>)delegate;
 ...
 * 设置业务错误(eg.用户名或密码不正确)的callback（不设置默认弹出Toast）
 */
- (void)setHandleBizNSErrorCallback:(void (^)(NSError* error))bizErrCallback;
